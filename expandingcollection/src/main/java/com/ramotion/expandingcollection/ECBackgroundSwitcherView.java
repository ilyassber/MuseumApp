...
//    }
    ...
    public void cacheBackgroundAtPosition(ECPager pager, int position) {
        if (position >= 0 && position < pager.getAdapter().getCount()) {
            Bitmap mainBgImageResourceBitmap = pager.getDataFromAdapterDataset(position).getMainBackgroundResourceBitmap();
            Integer mainBitmapKey = pager.getDataFromAdapterDataset(position).getMainBitmapKey();
            if (mainBgImageResourceBitmap == null) return;
            BitmapWorkerTask addBitmapToCacheTask = new BitmapWorkerTask(mainBitmapKey, mainBgImageResourceBitmap); //getResources(),
            addBitmapToCacheTask.execute(mainBitmapKey);
        }
    }
    ...
    public void updateCurrentBackground(ECPager pager, final AnimationDirection direction) {
        ...
        Integer mainBitmapKey = pager.getDataFromAdapterDataset(position).getMainBitmapKey();
        Bitmap cachedBitmap = instance.getBitmapFromBgMemCache(mainBitmapKey);
        if (cachedBitmap == null) {
            if (mainBitmapKey == null) return;
            cachedBitmap = pager.getDataFromAdapterDataset(position).getMainBackgroundResourceBitmap(); //BitmapFactory.decodeResource(getResources(), mainBgImageDrawableResource, new BitmapFactoryOptions());
            instance.addBitmapToBgMemoryCache(mainBitmapKey, cachedBitmap);
        }
    ...
    public void updateCurrentBackgroundAsync(ECPager pager, final AnimationDirection direction) {
        ...
        int position = pager.getCurrentPosition();
        //Integer mainBgImageDrawableResource = pager.getDataFromAdapterDataset(position).getMainBackgroundResource();
        Bitmap mainBgImageDrawableResourceBitmap = pager.getDataFromAdapterDataset(position).getMainBackgroundResourceBitmap();
        Integer mainBitmapKey = pager.getDataFromAdapterDataset(position).getMainBitmapKey();
        if (mainBgImageDrawableResourceBitmap == null) return;
        mCurrentAnimationTask = new BitmapWorkerTask(mainBitmapKey, mainBgImageDrawableResourceBitmap) { //getResources(),
            @Override
        ...
        mCurrentAnimationTask.execute(mainBitmapKey);
    }
